...
<h1 class="h1 text-3xl @md:text-2xl @4xl:text-3xl @5xl:text-4xl @6xl:text-5xl @7xl:text-6xl">
	{labels.heading} <span class="font-bold text-primary-500">{currentTargetAudience}</span>
</h1>
<p class="text-xl @md:text-3xl @4xl:text-4xl @5xl:text-5xl mb-6">
	{labels.description}
</p>
<div class="flex flex-col items-center text-sm @md:text-lg @4xl:text-xl @5xl:text-2xl mb-6">
	<p class="mb-2">{labels.callToActionPrefix}</p>
	<p
		class="h2 flex items-center justify-center w-full h-12 max-w-5xl font-bold text-secondary-400 text-xl @md:text-4xl @4xl:text-5xl"
	>
		{currentCallToAction}
	</p>
	<p class="mt-2">{labels.callToActionSuffix}</p>
</div>
<button
	type="button"
