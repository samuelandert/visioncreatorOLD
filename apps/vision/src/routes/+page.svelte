<script>
	...
	let paragraphs = [
		...
		'über 11 verschiedene Einkommensquellen habe, die mich und meine Familie',
		'ein ereignisreiches und gutes Leben führen lassen',
		'Ich bin dankbar und glücklich darüber, dass ich 2023 Visionsentwickler geworden bin.',
		...
		'auf unserem Planeten Tag für Tag für jeden Menschen immer geiler zu machen',
		'Heute ist ein besonderer Tag. In einer Stunde ist es so weit',
		'dass wir zum ersten mal eine Vision mit 1 Milliarde € finanzieren werden',
		...
		'und mich dazu entschieden habe Visionsentwickler zu sein',
		'In tiefer Dankbarkeit dein Visionsentwickler xyz'
	];
	let state = 1;
...
<div class="video-container">
	...
	<div class="absolute z-10 flex justify-around w-full text-2xl top-5">
		<div class="text-center">
			<img class="transparent-image" src="logo.png" alt="visioncreator" width="100px" />
		</div>
	</div>
		...
		{#if state === 1}
			<div class="max-w-4xl mx-auto text-center">
				<p class="max-w-2xl mx-auto mb-4 text-3xl text-white">
					{#if name}
						{name}, bist du bereit endlich deine Lebensvision so zu leben, wie du es dir immer
						gewünscht hast?
					{:else}
						Bist du bereit endlich deine Lebensvision so zu leben, wie du es dir immer gewünscht
						hast?
					{/if}
				</p>
				<div class="mb-8 text-center">
					<p class="p-4 text-xl text-gray-100">Nächster Milestone</p>
					<div class="w-full h-4 bg-white bg-opacity-50 rounded-full">
						<div
							class="h-full text-xs text-center text-white rounded-full bg-blue-950"
							style="width: 85.6%;"
						>
							...
							<div class="w-1/6 bg-white" />
							<div class="w-1/6 bg-white" />
							<div class="w-1/6 bg-white opacity-50" />
						</div>
					</div>
				...
				<p class="max-w-2xl mx-auto mb-2 text-lg text-center text-white">
					Zusammen machen wir unser Leben von Tag zu Tag immer geiler und knacken dabei den
					Crowdfunding Weltrekord und finanzieren so jeden Monat 1 visionäres Startup aus unserer
					Mitte heraus mit 1 Milliarde Euro an dem wir alle beteiligt sind.
				</p>

				<button class="px-4 py-2 mt-2 text-white bg-blue-900 rounded-2xl" on:click={next}
					>Ich bin bereit</button
				>
			</div>
		{:else if state === 2}
			<div style="display: flex; flex-direction: column; align-items: center; ">
				<input
					...
					style="background: rgba(255, 255, 255, 0.2); padding: 0.5rem 1rem; font-size: 1.25rem; color: white; border: 1px solid white; border-radius: 1rem; outline: none;"
				/>
				<button class="px-4 py-2 mt-2 text-white bg-blue-900 rounded-2xl" on:click={next}
					>Lass mich träumen</button
				>
			</div>
		{:else if state === 3}
			<div style="display: flex; flex-direction: column; align-items: center;">
				...
				<button class="px-4 py-2 mt-2 text-white bg-teal-500 rounded-2xl" on:click={next}
					>Auf gehts!</button
				>
			</div>
		{:else}
