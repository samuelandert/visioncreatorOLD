...
export function createDynamicMapping({ components, filter }) {
    const dynamicImportMapping = {};
    const componentPaths = [];
    ...
    makeArray(components).forEach(async comp => {
      ...
      traverse(componentPath, filter, name => {
        let moduleName = getModuleName(componentPath, name, flat, prefix);
        dynamicImportMapping[moduleName] = `() => import('${normalizePath(path.relative(componentPath, name))}')`;
      });
    });
  
    return [
      dynamicImportMapping,
      componentPaths,
    ];
  }
